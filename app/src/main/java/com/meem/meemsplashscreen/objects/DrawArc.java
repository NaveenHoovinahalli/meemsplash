...
public class DrawArc{
	...
	public DrawArc(){
		...
		for (int i = 1; i < 366; i++) {
			_vertices[(i * 3) + 0] = (float) (0.0225 * Math.cos((3.14 / 180) * (float) i));
			_vertices[(i * 3) + 1] = (float) (0.0225 * Math.sin((3.14 / 180)	* (float) i));
			_vertices[(i * 3) + 2] = 0;
		}
		...
		for (int i = 1; i < 366; i++) {
			_normals[(i * 3) + 0] = (float) (0.0225 * Math.cos((3.14 / 180) * (float) i));
			_normals[(i * 3) + 1] = (float) (0.0225 * Math.sin((3.14 / 180) * (float) i));
			_normals[(i * 3) + 2] = 0;
		} 
...
//			setMaterial(0.94f,0.96f, 0.47f, 1.0f);
		...
		GLES20.glDrawArrays(GLES20.GL_TRIANGLE_FAN, 0, 366);
	}
	...
	private void setMaterial(float r,float g,float b,float a) {
		...
		GLES20.glUniform4f(DrawArc._materialAmbientHandle,r,g,b,a);
		GLES20.glUniform4f(DrawArc._materialDiffuseHandle,r,g,b,a);
		GLES20.glUniform4f(DrawArc._materialSpecularHandle,r,g,b,a);
		GLES20.glUniform1f(DrawArc._materialShininessHandle, 0.6f);
	}
