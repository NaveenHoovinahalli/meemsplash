...
public class DrawBlackCircle {

	private float _vertices[] = new float[720 * 3];
	private float _normals[] = new float[720 * 3];
	...
	public DrawBlackCircle(){
		...
		for (int i = 1; i < 720; i++) {
			_vertices[(i * 3) + 0] = (float) (0.17375 * Math.cos((3.14 / 180) * (float) i));
			_vertices[(i * 3) + 1] = (float) (0.17375 * Math.sin((3.14 / 180)	* (float) i));
			_vertices[(i * 3) + 2] = 0;
		}
		...
		for (int i = 1; i < 720; i++) {
			_normals[(i * 3) + 0] = (float) (0.17375 * Math.cos((3.14 / 180) * (float) i));
			_normals[(i * 3) + 1] = (float) (0.17375 * Math.sin((3.14 / 180) * (float) i));
			_normals[(i * 3) + 2] = 0;
		} 
	...
	public void draw(float[] mvpMatrix, boolean isEoneLight) {
		...
		if(isEoneLight){
			setMaterial(0.94f,0.96f, 0.47f, 1.0f);
		}else
		{
...
//				setMaterial(_dcolor[0], _dcolor[1], _dcolor[2], 1.0f);
		...
		GLES20.glDrawArrays(GLES20.GL_TRIANGLE_FAN, 0, 720);
	}
	...
	private void setMaterial(float r,float g,float b,float a) {
		...
		GLES20.glUniform4f(DrawBlackCircle._materialAmbientHandle,r,g,b,a);
		GLES20.glUniform4f(DrawBlackCircle._materialDiffuseHandle,r,g,b,a);
		GLES20.glUniform4f(DrawBlackCircle._materialSpecularHandle,r,g,b,a);
		GLES20.glUniform1f(DrawBlackCircle._materialShininessHandle, 0.6f);
	}
